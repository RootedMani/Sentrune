...
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Mani_Jabari-0A66C2.svg?style=flat-square&logo=linkedin)](https://www.linkedin.com/in/mani-jabari-601312375/)
...
3. **Source-Attributed Financial NLP**: Local FinBERT item scoring with distinct tracking of curated/institutional versus ambient retail sentiment across rolling 24-hour, 72-hour, and 168-hour windows.
...
- **Embargo Gaps**: Because target labels represent forward returns over a horizon of 5 bars (`return = P(t+5) / P(t) - 1`), the final 5 bars of each training fold are explicitly purged before the test boundary begins. This guarantees that training labels never ingest future price movements that bleed into the validation period.
...
```text
Label(t) = 
  Down (Class 0) : forward_return < -0.5%
  Flat (Class 1) : -0.5% <= forward_return <= +0.5%
  Up   (Class 2) : forward_return > +0.5%
```
where `0.5%` represents the neutral dead-zone threshold (`delta = 0.005`). This allows models to express conviction in directional expansion while filtering out consolidation regimes.

### 3. Source-Attributed FinBERT Sentiment Aggregation
Rather than treating all social and news volume as homogeneous noise:
- Every headline or post is passed through [`ProsusAI/finbert`](https://huggingface.co/ProsusAI/finbert) to extract probability distributions: `[p_positive, p_neutral, p_negative]`.
- The normalized sentiment score is formulated as:  
  `Sentiment = p_positive - p_negative` (bounded in `[-1.0, +1.0]`).
...
- When joining sentiment to market bars, an **as-of timestamp join** is enforced: only aggregates where `window_end <= t` are eligible at bar timestamp `t`, strictly eliminating forward lookahead bias.
...
- **Exchange Fee**: 10 bps (0.10%) per executed trade.
- **Slippage & Spread**: 5 bps (0.05%) per transaction.
- **Round-Trip Friction**: 15 bps (0.15%) modeled cost deducted from gross returns before computing Sharpe Ratio, maximum drawdown, and win rates.
...
**Mani Jabari**  
- **LinkedIn**: [linkedin.com/in/mani-jabari-601312375](https://www.linkedin.com/in/mani-jabari-601312375/)  
