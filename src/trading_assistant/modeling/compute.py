...
def run(config_path: str = "config/modeling.yaml") -> dict:
    ...
    for asset in assets:
        try:
            ...
            run_id = conn.execute("INSERT INTO model_runs(asset_id,interval,model_name,trained_at,model_path,feature_columns) VALUES(?,?,?,?,?,?)", (asset["id"], settings.interval, "lightgbm", datetime.now(timezone.utc).isoformat(), result.get("model_path", ""), json.dumps(settings.feature_columns or DEFAULT_FEATURES))).lastrowid
