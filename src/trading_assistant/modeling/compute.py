...
TECHNICAL_FEATURES = [
    ...
    # Deliberately excludes ichimoku_chikou: it's a lagging span (close
    # shifted back ~26 periods), so by construction it is NaN for the most
    # recent ~26 bars of any history - including, always, the newest bar.
    # A feature that is *always* missing on the bar you actually want to
    # predict from is unusable for live prediction; it's still computed and
    # stored in technical_features for charting/display, just not selected
...
def train_asset(conn, asset_id: int, settings) -> dict:
    ...
    if settings.tune_hyperparameters:
        # Reuses the exact same purged, expanding folds as validation below -
        # never a shuffled search - so the chosen hyperparameters are not
        # selected using any information from outside their own training
        # window. See tuning/search.py for why log loss (not accuracy) is
        # the ranking metric: it rewards calibrated probabilities, which is
        # what the strategy layer actually acts on.
        ...
        if tuning_result.improved_over_default:
            ...
            log.info(
                "Asset %s: tuning improved mean log loss %.4f -> %.4f, using %s",
                asset_id, tuning_result.default_mean_log_loss, tuning_result.best_mean_log_loss, tuning_result.best_params,
            )
        else:
    ...
    for model_name, model in models.items():
        ...
        for fold, train_idx, test_idx in splits(len(dataset), settings.min_train_size, settings.test_size, settings.folds, purge=settings.horizon_bars):
            ...
            if model_name == "lightgbm":
                # Strategy backtest reuses this fold's already-purged test
                # window and already-fitted model - no extra fit/predict pass.
