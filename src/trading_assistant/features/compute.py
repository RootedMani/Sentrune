...
def _window_grid(item_timestamps: list[datetime], window_hours: int) -> list[datetime]:
    """Return the window_end grid covering the item history.

    Windows are aligned to whole hours and advanced by window_hours so each
    asset gets a full time series of aggregates instead of only the latest
    window. The modeling layer's as-of join and the alerting layer's spike
    detection both need that history. The final partial window (ending at most
    one step after the newest item) is included so the newest mentions are
    covered.
    """
    ...
    for asset in assets:
        ...
        for hours in settings.sentiment_windows_hours:
            ...
            for window_end in grid:
                ...
                # Two-pointer over the sorted items keeps each run O(items)
                # overall instead of rescanning everything per window.
