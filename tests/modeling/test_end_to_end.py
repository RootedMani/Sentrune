...
def _make_db(path: Path):
    ...
    CREATE TABLE sentiment_aggregates (id INTEGER PRIMARY KEY, asset_id INTEGER, window_end TEXT, window_hours INTEGER, avg_sentiment REAL);
    ...
    for i, timestamp in enumerate(dates):
        ...
        conn.execute("INSERT INTO sentiment_aggregates(asset_id,window_end,window_hours,avg_sentiment) VALUES(?,?,?,?)", (1, timestamp.isoformat(), 24, 0.1))
    conn.commit()
    conn.close()
